...
const SimulationResults: React.FC<SimulationResultsProps> = ({ 
  ...
  const colors = samplePaths.map((_, index) => 
  ...
  const chartData: ChartData<'line'> = {
    ...
    datasets: [
      ...
      ...samplePaths.map((path, index) => ({
        label: (index === firstSuccessfulIndex && firstSuccessfulIndex !== -1) 
          ? 'Successful Paths' 
    ...
    <div className="simulation-results">
      ...
      <div className="result-summary">
        <div className="result-card probability">
          ...
          <div className="result-value">{((totalPaths - successPathsCombined) / totalPaths * 100).toFixed(2)}%</div>
        ...
        <div className="result-card upper-bound">
          ...
          <div className="result-detail">
            {successPathsUpper} paths hit upper bound
          </div>
        </div>
        ...
        <div className="result-card lower-bound">
          ...
          <div className="result-detail">
            {successPathsLower} paths hit lower bound
          </div>
        </div>
      </div>
      ...
      <div className="interpretation">
        ...
        <p>
          Based on the Monte Carlo simulation with {totalPaths} iterations, 
          starting from a current price of <strong>{currentPrice}</strong> with 
          a range of <strong>±{rangePrice}</strong> and Average True Range (ATR) of {paths.length > 0 ? Math.abs(paths[0][1] - paths[0][0]).toFixed(2) : 'N/A'}:
        </p>
        <ul>
          <li>The price has a <strong>{probabilityCombined.toFixed(2)}%</strong> probability of reaching either bound within {days} days.</li>
          <li>The probability of hitting the upper bound ({upperBound.toFixed(2)}) is <strong>{probabilityUpper.toFixed(2)}%</strong>.</li>
          <li>The probability of hitting the lower bound ({lowerBound.toFixed(2)}) is <strong>{probabilityLower.toFixed(2)}%</strong>.</li>
        </ul>
        {successPathsUpper > 0 && (
          <p>
            For the upper bound, it was reached in as few as {upperTargetStats.min} days,
            with an average of {upperTargetStats.avg.toFixed(1)} days.
          </p>
        )}
        {successPathsLower > 0 && (
          <p>
            For the lower bound, it was reached in as few as {lowerTargetStats.min} days,
            with an average of {lowerTargetStats.avg.toFixed(1)} days.
          </p>
        )}
      </div>
    </div>
